with constants as (
	...
	select
		...
		count(distinct o2.id) filter (where o2.workflow_state = 'canceled' 
			...
			and o2.deal_id is null
			and o2.confirmed_at is not null
			and c.cancel_type like 'ivr%') as canceled_ivr_nt,
			
		count(distinct o2.id) filter (where o2.workflow_state = 'canceled' 
			and o2.canceled_at < o1.start_at
			and o2.deal_id is null
			and o2.confirmed_at is not null
			and c.cancel_type = 'subscr_cancel') as canceled_subscr_nt,
		...
		count(distinct o2.id) filter (where o2.workflow_state = 'canceled' 
			...
			and o2.deal_id is null
			and o2.confirmed_at is not null
			and c.cancel_type = 'part-subscr') as canceled_partsubscr_nt,
		...
		min(date(o1.start_at)-date(o2.canceled_at)) filter (where o2.workflow_state = 'canceled' 
			...
			and o2.deal_id is null
			and o2.confirmed_at is not null
			and c.cancel_type like 'ivr%') as days_since_last_cancel_ivr_nt,
			
		min(date(o1.start_at)-date(o2.canceled_at)) filter (where o2.workflow_state = 'canceled' 
			and o2.canceled_at < o1.start_at
			and o2.deal_id is null
			and o2.confirmed_at is not null
			and c.cancel_type = 'subscr_cancel') as days_since_last_cancel_subscr_nt,
		...
		min(date(o1.start_at)-date(o2.canceled_at)) filter (where o2.workflow_state = 'canceled' 
			...
			and o2.deal_id is null
			and o2.confirmed_at is not null
			and c.cancel_type = 'part-subscr') as days_since_last_cancel_partsubscr_nt

		
	from orders o1
		left join orders o2 on (o1.user_id = o2.user_id and o2.start_at < o1.start_at)
		left join cancel_classification c on (o1.id = c.order_id)
	...
	select
		...
		case when pets then 1 else 0 end as pets,
		case when need_vacuum_cleaner then 1 else 0 end as need_vacuum_cleaner,
		...
		case when extract(hour from start_at) between 9 and 12 then 1 else 0 end day_slot,
...
select 
	...
	canceled_subscr_nt,
	canceled_onetime_nt,
	canceled_partsubscr_nt,
	days_since_last_cancel_all_nt,
	days_since_last_cancel_ivr_nt,
	days_since_last_cancel_subscr_nt,
	days_since_last_cancel_onetime_nt,
	days_since_last_cancel_partsubscr_nt	
